...
# Read the actual CSV file (adjust the file path accordingly)
nj_diabetes_data <- read.csv("combined_data.csv", check.names = FALSE, header = FALSE)

# Rename the columns manually, ensuring there are no NAs
...
# Remove any empty columns or columns with 'NA' as their names
...
print(colnames(nj_diabetes_data))

# Ensure that there are no empty strings or NA values in column names
colnames(nj_diabetes_data) <- gsub("^\\s*$", "Unknown", colnames(nj_diabetes_data))  # Replace empty strings with "Unknown"
colnames(nj_diabetes_data) <- na.omit(colnames(nj_diabetes_data))  # Remove any NA column names
